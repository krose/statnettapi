...
#' @export
#'
stat_frequency_minute <- function(from_date, date_as_cet = FALSE){

  if(!lubridate::is.Date(from_date)){
    stop("The from_date param is not of class date.")
  }

  data_url <- paste0("http://driftsdata.statnett.no/restapi/Frequency/ByMinute?From=", from_date)
  ...
  tibble::tibble(date = date_utc, measurement = data_content$Measurements)
}
